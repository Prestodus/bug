...
$name = "Clean"; // Name of the template (Note that there can only exist 1 template with this name in the database, else it will be ignored)
$tplpath = "clean"; // Path to the template in ./templates/ (example: default leads to ./templates/default/)
$version = "1.0"; // Version of the template
$about = "This template is a cleaner template for my framework."; // A short note about the template, as shown in the admin panel, use \n for line breaks
$vars = "sidebar:1,prefooter:1,slogan:1,sitename:1,menu:1,logo:1,rightbox:1,preheader:1"; // The variables as explained above, seperated by a comma (,) (Example: if you created a second menu called menu2, and you want the administrator to have a choice whether to show or hide it, add ",menu2:1")
...
if ($sql->fetchColumn() < 1) {
    ...
    $templateid = $dbh->lastInsertId();
    $sql = "INSERT INTO ".DB_PREFIX."textblock (template, name, text) VALUES (?, ?, ?)";
    $insert = $dbh->prepare($sql);
    $insert->execute(array($templateid, "headertext", NULL));
